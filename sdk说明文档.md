...
| 场景     | 名称 / 路径                                | 类型      | 用途简述                             |
|----------|--------------------------------------------|-----------|--------------------------------------|
| 授权登录 | `getTokenAndLogin()`                       | 前端 Hook | 一行代码完成虎扑授权 + 后端登录      |
| 授权登录 | `/login/otherHupuH5Login`                  | 后端 HTTP | 根据虎扑 `accessToken` 换游戏登录态  |
| 确认角色 | `/user/chooseRole`                         | 后端 HTTP | 上报并确认当前角色信息               |
| 商品信息 | `/order/getProductInfo`                    | 后端 HTTP | 根据 SKU 等获取商品价格信息          |
| 支付     | `reCharge`（封装在 `purchaseProduct` 里）  | 虎扑 SDK  | 拉起虎扑支付收银台                   |
| 查余额   | `getBalance()`                             | 虎扑 SDK  | 查询虎扑账户余额                     |

> 只要搞清楚这几条，就可以完成「登录 + 角色确认 + 内购」的接入。
...
- **调用结果（关键点）**：
  - `localStorage.hupu_token`：保存游戏登录 token，后续所有接口会自动带上。
  - `loginResult`：由你们后端定义，一般至少包含：
    - `token`：游戏登录态
    - 其他字段按你们实际业务返回即可。
